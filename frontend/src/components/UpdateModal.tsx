...
import { updateTodo } from "@/hooks/useTodo";
import { Todo } from "@/types/todo";

type Props = {
  todo: Todo;
};
export default function UpdateModal({ todo }: Props) {
  const [formTodo, setFormTodo] = useState({
    name: todo.name,
    description: todo.description,
    priority: todo.priority,
    timeRelease: todo.timeRelease,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const validateResult = await updateTodo(todo._id, {
        name: formTodo.name,
